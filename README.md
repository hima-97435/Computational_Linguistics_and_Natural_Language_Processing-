...
Follow these steps to upload your repository to GitHub. The `.gitignore` file created in this repository will automatically ensure that `venv/`, `.vscode/`, `main.py`, and `main.dSYM/` directories are **not** committed or uploaded.

### Step 1: Initialize Git
Open your terminal in the project directory (`NLP Lab`) and initialize the repository:
```bash
git init
```

### Step 2: Add Files to Staging
Add all project files. The `.gitignore` rules will automatically exclude the files/folders you want to avoid adding:
```bash
git add .
```

### Step 3: Check Status (Optional but Recommended)
Verify that only the intended files are staged (you should see files from `Lab1/`, `LAB2/`, `.gitignore`, and `README.md` in green, and no trace of `venv/`, `main.py`, `.vscode/`, or `main.dSYM/`):
```bash
git status
```
...
```bash
git commit -m "Initial commit: Add README, Lab 1 Data Analysis, and Lab 2 NLP Preprocessing notebooks"
```

### Step 5: Configure Branch and Remote
Set the default branch to `main` and link your local repository to your remote GitHub repository:
